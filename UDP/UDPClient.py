...
while True:
    ...
    continue_conversing = input("Do you want to continue conversing? Type \"exit\" to stop, else type your message.\n")
    if continue_conversing == 'exit':
        print("RUDE!")
        clientSocket.close()
        break
    else:
        message = continue_conversing
        messageBytes = message.encode("utf-8")
        clientSocket.sendto(messageBytes, serverAddress)
